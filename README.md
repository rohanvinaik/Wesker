...
The mutation theory formalizes this via **predictive priors** (Layer 2 of the Monty Hall architecture, §6.2). Each Wesker run writes per-category aggregate survival statistics to `.wesker/mutation_report.json`. On subsequent runs, these historical rates are loaded automatically and used to order categories by descending survival probability. A project where ARITHMETIC mutants historically survive at 40% while VALUE mutants are always killed will test ARITHMETIC first — spending budget where specification gaps actually exist, not where the test suite is already strong.
